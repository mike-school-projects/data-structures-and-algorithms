...
const toTitleCase = (arr) => {
  ...
  return arr.map((element) => {
    tempString = '';
    tempString += element.charAt(0).toUpperCase();
    tempString += element.slice(1, element.length);
    return tempString;
  });
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    board.forEach((row) => {
      rowResults = true;
      row.forEach((element) => {
        if (element !== test) rowResults = false;
      });
      if (rowResults === true) results = true;
    });
    ...
    for (let column = 0; column < 3; column++) {
      ...
      for (let row = 0; row < 3; row++) {
        if (board[row][column] !== test) columnResults = false;
      }
      if (columnResults === true) results = true;
    }

    // text for across
    if (board[1][1] === test) {
      if (board[0][0] === test && board[2][2] === test) results = true;
      if (board[0][2] === test && board[2][0] === test) results = true;
    }
