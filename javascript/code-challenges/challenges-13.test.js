...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    if (i % 2 === 1) tempString += str.charAt(i);
  }
  return tempString;
};
...
const allHappy = (arr) => {
  let isHappy = true;
  arr.forEach((element) => {
    if (!element.includes(':)')) isHappy = false;
  });
  return isHappy;
};
...
const findEvery = (arr, target) => {
  let hasString = true;
  arr.forEach((element) => {
    if (!element.includes(target)) hasString = false;
  });
  return hasString;
};
...
const unenrollBrook = (arr) => arr.map((elementOuter) => elementOuter.filter((elementInner) => !elementInner.includes('Brook')));
...
const sortByDay = (arr) => daysOfWeek.map((day) => arr.filter((element) => element.includes(day)));
...
const characterByIndex = (arr) => arr.map((word, i) => word.charAt(i));
