...
const validateEmail = (email) => {
  ...
  let regexDomain = /@[a-z,A-Z].*?(\.)(com|net|org)$/;
  let regexTwoNames = /[a-z,A-Z](\.)[a-z,A-Z].*?@/;
  let numPeriods = 0;

  for (let i = 0; i < email.length; i++) {
    email.charAt(i) === '.' ? numPeriods++ : null;
  }

  numPeriods > 2 ? answer = false : null;

  if (numPeriods === 2) {
    regexTwoNames.test(email) ? null : answer = false;
  }

  regexName.test(email) ? null : answer = false;
  regexDomain.test(email) ? null : answer = false;
...
const validatePhoneNumber = (phoneNumber) => {
  let answer = true;
  let regexNumber = /[0-9]/;
  let regexParenthesis = /^\([0-9]{3}\)(-| |)[0-9]{3}(-| )[0-9]{4}$/;
  let regex2 = /^[0-9]{3}(-| |)[0-9]{3}(-| |)[0-9]{4}$/;
  let count = 0;

  for (let i = 0; i < phoneNumber.length; i++) {
    if (regexNumber.test(phoneNumber.charAt(i))) {
      count++;
    }
  }
  count === 10 ? null : answer = false;



  if (regexParenthesis.test(phoneNumber)) {
    return answer;
  } else regex2.test(phoneNumber) ? null : answer = false;
...
// console.log(validatePhoneNumber('555-555-5555'));
// console.log(validatePhoneNumber('555 555 5555'));
// console.log(validatePhoneNumber('555555-5555'));
